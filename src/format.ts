...
export function generateCompactText(data: SajuResult): string {
  ...
  // ## 기본
  lines.push(`## 기본`);
  lines.push(`${data.input.year}.${pad2(data.input.month)}.${pad2(data.input.day)} ${pad2(data.input.hour)}:${pad2(data.input.minute)} ${data.input.gender} ${calendarKo} ${data.input.timezone} ${data.currentAge}세`);
